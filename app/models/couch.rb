...
def is_free?(ida, vuelta)
    ...
    reservas_confirmadas.where('? < fin', ida) # from se copiaría en el lugar de '?'
    reservas_confirmadas.where('? > inicio', vuelta) # to se copiaría en el lugar de '?'
...
def self.free_couches(tipo, lugar, capacidad, puntuacion, ida, vuelta)
